...
export function ThemeContextProvider({ children }: ThemeContextProviderProps): JSX.Element {
  ...
  const { user, setUserTheme, setUserAccent } = useAuth();
  ...
  useEffect(() => {
    const flipTheme = (theme: Theme): NodeJS.Timeout | undefined => {
      ...
      root.style.setProperty("--main-background-start-rgb", `var(--${theme}-background-start-rgb)`);

      root.style.setProperty("--main-background-end-rgb", `var(--${theme}-background-end-rgb)`);

      root.style.setProperty("--main-background", `var(--${theme}-background)`);

      root.style.setProperty("--main-text", `var(--${theme}-text)`);

      root.style.setProperty("--main-border", `var(--${theme}-border)`);
      ...
      if (user) {
        ...
        return setTimeout(() => void setUserTheme(theme), 500);
      }
  ...
  useEffect(() => {
    const flipAccent = (accent: Accent): NodeJS.Timeout | undefined => {
      ...
      root.style.setProperty("--main-accent", `var(--accent-${accent}) !important`);

      if (user) {
        localStorage.setItem("accent", accent);
        return setTimeout(() => void setUserAccent(accent), 500);
      }
